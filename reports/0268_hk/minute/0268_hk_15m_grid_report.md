...
| 问题 | 样本内 | 样本外 | 怎么理解 |
| --- | --- | --- | --- |
| 这套策略能不能赚钱 | -1.61% | -0.33% | 当前还不能证明这套网格能稳定盈利，尤其要继续观察单边下跌时未平仓风险如何处理。 |
| 比现金闲置好不好 | -3217.73 | -667.51 | 正数表示网格策略赚到钱，负数表示不交易反而更好。 |
| 比买入持有好不好 | 39816.88 | 17234.79 | 买入持有用同样资金、交易单位和执行口径估算，正数表示网格更好。 |
| 交易成本高不高 | 424.37 | 76.81 | 这里统计手续费，滑点会单独体现在估算成交价和滑点成本里。 |
| 最坏会亏到什么程度 | 4.53% | 1.10% | 这是账户在样本期间相对阶段高点出现过的最大回撤。 |
| 这组参数稳不稳 | 稳健分 -0.36 | 沿用同一组参数 | 不是只看一整段最高分，而是看多窗口表现是否稳定。当前结果：67% 窗口为正，最差窗口收益 `-0.41%`，收益波动 `1.04` 个百分点。 |
...
| 指标 | 样本内 | 样本外 | 怎么读 |
| --- | --- | --- | --- |
| 净收益率 | -1.61% | -0.33% | 已经按当前执行口径扣除回测引擎支持的费用影响。 |
| 最大回撤 | 4.53% | 1.10% | 再看亏起来最难受会到什么程度。 |
| 交易成本 | 424.37 | 76.81 | 策略内部估算的手续费累计值，帮助判断网格频繁交易是否吃掉收益。 |
| 滑点成本 | 106.09 | 19.20 | 按收盘价和估算成交价差额累计，属于近似实盘口径。 |
| 未平网格有效成本 | 0.00 | 7.71 | 只在期末仍有未平网格仓位时有意义。 |
| 闭环网格净利润 | -3270.46 | 851.04 | 这是已经完成低买高卖、真正落袋的利润，不等于总账户收益。 |
| 未平网格浮动盈亏 | 0.00 | -1592.58 | hold 口径会保留这部分风险，force_exit 口径触发后通常回到 0。 |
| 网格闭环次数 | 10 | 1 | 次数越多，说明震荡里成交越频繁；但次数多不等于总账户一定赚钱。 |
...
| 买入持有收益率 | -21.52% | -8.95% |
| 网格策略收益率 | -1.61% | -0.33% |
| 网格相对现金闲置多赚/多亏 | -3217.73 | -667.51 |
| 网格相对买入持有多赚/多亏 | 39816.88 | 17234.79 |

### 左侧行情怎么处理

| 左侧口径 | 样本内净收益率 | 样本内闭环利润 | 样本内浮动盈亏 | 样本内强平 | 样本外净收益率 | 样本外闭环利润 | 样本外浮动盈亏 | 样本外强平 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| hold：未平网格继续持有 | -1.20% | 7337.20 | -9944.68 | 否 | -0.33% | 851.04 | -1592.58 | 否 |
| force_exit：达到亏损阈值强平 | -1.61% | -3270.46 | 0.00 | 是 | -0.33% | 851.04 | -1592.58 | 否 |
...
- 这一段价格从 `10.81` 走到 `8.43`，区间涨跌幅约 `-22.02%`。
...
- 样本外账户最终从 `200000` 走到 `199332.49`，总盈亏 `-667.51`。
- 样本外单层网格按最小交易单位 `1000` 股取整，固定数量是 `3000` 股。
- 样本外没有转正，说明这组参数还不能在该行情结构下独立制造稳定盈利。

#### 样本外回测图

- 这一段价格从 `8.36` 走到 `7.59`，区间涨跌幅约 `-9.21%`。
- 样本结束时收盘价 `7.59` 仍低于有效成本 `7.71`，未平网格还处在约 `1.51%` 的浮亏区。
- 图里的买卖点一共完成了 `1` 轮网格闭环，已经落袋的网格利润累计 `851.04`。
- 期末未平网格浮动盈亏为 `-1592.58`。
- 总账户最终仍是亏损状态，期末权益 `199332.49`；也就是说，已实现网格利润还没完全覆盖未平仓或强制退出带来的亏损。
...
| 时间 | 事件类型 | 层级 | 价格 | 估算成交价 | 数量 | 金额 | 手续费 | 滑点成本 | 说明 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-05-18 02:30:00 | grid_buy | 1 | 8.01 | 8.01 | 3000 | 24054.03 | 19.23 | 4.81 | 触发下行网格买入 |
| 2026-05-19 01:30:00 | grid_sell | 1 | 8.31 | 8.31 | 3000 | 24905.08 | 19.94 | 4.99 | 达到网格止盈价后卖出本层仓位 |
| 2026-05-20 01:30:00 | grid_buy | 1 | 8.01 | 8.01 | 3000 | 24054.03 | 19.23 | 4.81 | 触发下行网格买入 |
...
| 开仓时间 | 平仓时间 | 持有时长 | 开仓价 | 平仓价 | 数量 | 盈亏 | 收益率(%) | 仓位类型 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-05-18 02:30:00 | 2026-05-19 01:30:00 | 0 days 23:00:00 | 8.01 | 8.31 | 3000 | 856.02 | 3.56 | 网格 1 |
| 2026-05-20 01:30:00 | 2026-05-21 08:00:00 | 1 days 06:30:00 | 8.01 | 7.60 | 3000 | -1272.27 | -5.29 | 网格 1 |
| 2026-05-21 02:45:00 | 2026-05-21 08:00:00 | 0 days 05:15:00 | 7.67 | 7.60 | 3000 | -251.25 | -1.09 | 网格 2 |

## 最终结论

- 这套参数更适合“先跌一段、再进入震荡或反弹”的行情，因为它依赖反弹来兑现网格利润。
- 如果行情持续单边下跌，hold 口径会继续持有未平网格，force_exit 口径会在浮亏达到阈值后清仓并停止交易。
- 当前样本下，闭环网格净利润：样本内 -3270.46，样本外 851.04。
