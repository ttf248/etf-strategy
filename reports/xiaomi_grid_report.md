...
- 样本内收益率：-14.21%
  - 人话：整个账户从 `200000` 变成了 `170008.00`，合计亏损 `29992.00`。
- 样本内年化收益率：-37.56%
  - 人话：这是把这段样本期的结果折算成年化后的速度，用来和别的策略比较，不是说你真的持有满一年亏这么多。
- 样本内最大回撤：19.51%
  - 人话：样本里最差的时候，账户相对阶段最高点最多回撤了约 `19.51%`。
- 期末有效持仓成本：51.86
  - 人话：把已经通过网格落袋的利润扣掉后，你手里剩余持仓等效成本大约是 `51.86`。
- 相对初始建仓成本下降：7.15%
  - 人话：最开始底仓买在 `55.85`，现在等效成本被网格摊低了约 `7.15%`。
- 网格已实现收益：984.00
  - 人话：单看“低买高卖已经完成并落袋”的网格交易，本轮一共赚了 `984.00`。
- 完成网格循环次数：3
...
- 样本外收益率：-11.84%
  - 人话：整个账户从 `200000` 变成了 `174336.00`，合计亏损 `25664.00`。
- 样本外年化收益率：-28.92%
- 样本外最大回撤：13.88%
- 样本外沿用最小交易单位：200 股
- 样本外单层网格固定数量：400 股
...
- 相对样本外首笔建仓成本下降：7.48%
- 网格已实现收益：3344.00
  - 人话：单看已经完成并兑现的网格交易，样本外一共落袋赚了 `3344.00`。
- 完成网格循环次数：5
