...
| 问题 | 样本内 | 样本外 | 怎么理解 |
| --- | --- | --- | --- |
| 这套策略能不能赚钱 | -1.80% | 0.00% | 当前还不能证明这套网格能稳定盈利，尤其要继续观察单边下跌时未平仓风险如何处理。 |
| 比现金闲置好不好 | -3590.12 | 0.00 | 正数表示网格策略赚到钱，负数表示不交易反而更好。 |
| 比买入持有好不好 | 34764.70 | 3649.23 | 买入持有用同样资金、交易单位和执行口径估算，正数表示网格更好。 |
| 交易成本高不高 | 481.75 | 0.00 | 这里统计手续费，滑点会单独体现在估算成交价和滑点成本里。 |
| 最坏会亏到什么程度 | 4.11% | 0.00% | 这是账户在样本期间相对阶段高点出现过的最大回撤。 |
| 这组参数稳不稳 | 稳健分 -0.48 | 沿用同一组参数 | 不是只看一整段最高分，而是看多窗口表现是否稳定。当前结果：100% 窗口为正，最差窗口收益 `0.07%`，收益波动 `0.16` 个百分点。 |
...
| 指标 | 样本内 | 样本外 | 怎么读 |
| --- | --- | --- | --- |
| 净收益率 | -1.80% | 0.00% | 已经按当前执行口径扣除回测引擎支持的费用影响。 |
...
| 未平网格浮动盈亏 | -7203.73 | 0.00 | hold 口径会保留这部分风险，force_exit 口径触发后通常回到 0。 |
...
| 买入持有收益率 | -19.18% | -1.82% |
| 网格策略收益率 | -1.80% | 0.00% |
| 网格相对现金闲置多赚/多亏 | -3590.12 | 0.00 |
| 网格相对买入持有多赚/多亏 | 34764.70 | 3649.23 |

### 左侧行情怎么处理

| 左侧口径 | 样本内净收益率 | 样本内闭环利润 | 样本内浮动盈亏 | 样本内强平 | 样本外净收益率 | 样本外闭环利润 | 样本外浮动盈亏 | 样本外强平 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| hold：未平网格继续持有 | -1.80% | 3748.23 | -7203.73 | 否 | 0.00% | 0.00 | 0.00 | 否 |
| force_exit：达到亏损阈值强平 | -1.80% | 3748.23 | -7203.73 | 否 | 0.00% | 0.00 | 0.00 | 否 |
...
- 这一段价格从 `33.60` 走到 `26.66`，区间涨跌幅约 `-20.65%`。
- 样本结束时收盘价 `26.66` 仍低于有效成本 `28.36`，未平网格还处在约 `6.00%` 的浮亏区。
- 图里的买卖点一共完成了 `18` 轮网格闭环，已经落袋的网格利润累计 `3748.23`。
- 期末未平网格浮动盈亏为 `-7203.73`。
- 总账户最终仍是亏损状态，期末权益 `196409.88`；也就是说，已实现网格利润还没完全覆盖未平仓或强制退出带来的亏损。
...
- 这一段价格从 `26.56` 走到 `26.10`，区间涨跌幅约 `-1.73%`。
...
| 2026-02-27 01:30:00 | 2026-04-30 01:30:00 | 62 days 00:00:00 | 32.21 | 26.56 | 500 | -2846.73 | -17.68 | 网格 1 |
| 2026-03-27 06:00:00 | 2026-04-30 01:30:00 | 33 days 19:30:00 | 30.91 | 26.56 | 500 | -2196.08 | -14.21 | 网格 2 |
| 2026-03-30 05:45:00 | 2026-04-30 01:30:00 | 30 days 19:45:00 | 29.51 | 26.56 | 500 | -1495.38 | -10.14 | 网格 3 |
| 2026-04-20 03:45:00 | 2026-04-30 01:30:00 | 9 days 21:45:00 | 28.23 | 26.56 | 500 | -854.74 | -6.06 | 网格 4 |
