...
| 问题 | 样本内 | 样本外 | 怎么理解 |
| --- | --- | --- | --- |
| 这套策略能不能赚钱 | -4.47% | -1.38% | 当前还不能证明这套网格能稳定盈利，尤其要继续观察单边下跌时未平仓风险如何处理。 |
| 比现金闲置好不好 | -8941.56 | -2767.39 | 正数表示网格策略赚到钱，负数表示不交易反而更好。 |
| 比买入持有好不好 | 47895.91 | 21625.80 | 买入持有用同样资金、交易单位和执行口径估算，正数表示网格更好。 |
| 交易成本高不高 | 267.27 | 57.56 | 这里统计手续费，滑点会单独体现在估算成交价和滑点成本里。 |
| 最坏会亏到什么程度 | 4.81% | 1.63% | 这是账户在样本期间相对阶段高点出现过的最大回撤。 |
| 这组参数稳不稳 | 稳健分 -4.61 | 沿用同一组参数 | 不是只看一整段最高分，而是看多窗口表现是否稳定。当前结果：33% 窗口为正，最差窗口收益 `-3.67%`，收益波动 `1.82` 个百分点。 |
...
| 稳健分 RobustScore | -4.61 | 计算方式：0.6 x 窗口平均分 + 0.4 x 最差窗口分 - 0.25 x 窗口收益波动。 |
...
| 指标 | 样本内 | 样本外 | 怎么读 |
| --- | --- | --- | --- |
| 净收益率 | -4.47% | -1.38% | 已经按当前执行口径扣除回测引擎支持的费用影响。 |
| 最大回撤 | 4.81% | 1.63% | 再看亏起来最难受会到什么程度。 |
| 交易成本 | 267.27 | 57.56 | 策略内部估算的手续费累计值，帮助判断网格频繁交易是否吃掉收益。 |
| 滑点成本 | 66.82 | 14.39 | 按收盘价和估算成交价差额累计，属于近似实盘口径。 |
| 未平网格有效成本 | 0.00 | 4.00 | 只在期末仍有未平网格仓位时有意义。 |
| 闭环网格净利润 | -8974.08 | 0.00 | 这是已经完成低买高卖、真正落袋的利润，不等于总账户收益。 |
| 未平网格浮动盈亏 | 0.00 | -2961.06 | hold 口径会保留这部分风险，force_exit 口径触发后通常回到 0。 |
...
| 买入持有收益率 | -28.42% | -12.20% |
| 网格策略收益率 | -4.47% | -1.38% |
| 网格相对现金闲置多赚/多亏 | -8941.56 | -2767.39 |
| 网格相对买入持有多赚/多亏 | 47895.91 | 21625.80 |

### 左侧行情怎么处理

| 左侧口径 | 样本内净收益率 | 样本内闭环利润 | 样本内浮动盈亏 | 样本内强平 | 样本外净收益率 | 样本外闭环利润 | 样本外浮动盈亏 | 样本外强平 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| hold：未平网格继续持有 | -8.55% | 1153.32 | -18439.08 | 否 | -1.38% | 0.00 | -2961.06 | 否 |
| force_exit：达到亏损阈值强平 | -4.47% | -8974.08 | 0.00 | 是 | -1.38% | 0.00 | -2961.06 | 否 |
...
- 这一段价格从 `6.17` 走到 `4.40`，区间涨跌幅约 `-28.69%`。
...
- 样本外账户最终从 `200000` 走到 `197232.61`，总盈亏 `-2767.39`。
- 样本外单层网格按最小交易单位 `2000` 股取整，固定数量是 `6000` 股。
- 样本外没有转正，说明这组参数还不能在该行情结构下独立制造稳定盈利。

#### 样本外回测图

- 这一段价格从 `4.39` 走到 `3.84`，区间涨跌幅约 `-12.53%`。
- 样本结束时收盘价 `3.84` 仍低于有效成本 `4.00`，未平网格还处在约 `4.02%` 的浮亏区。
- 这段区间里没有完成任何网格闭环，所以图上即使有持仓波动，也还没有形成已落袋的网格利润。
- 期末未平网格浮动盈亏为 `-2961.06`。
- 总账户最终仍是亏损状态，期末权益 `197232.61`；也就是说，已实现网格利润还没完全覆盖未平仓或强制退出带来的亏损。
...
| 时间 | 事件类型 | 层级 | 价格 | 估算成交价 | 数量 | 金额 | 手续费 | 滑点成本 | 说明 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-05-18 01:30:00 | grid_buy | 1 | 4.12 | 4.12 | 6000 | 24744.72 | 19.78 | 4.94 | 触发下行网格买入 |
...
| 开仓时间 | 平仓时间 | 持有时长 | 开仓价 | 平仓价 | 数量 | 盈亏 | 收益率(%) | 仓位类型 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-05-18 01:30:00 | 2026-05-21 08:00:00 | 3 days 06:30:00 | 4.12 | 3.85 | 6000 | -1663.20 | -6.73 | 网格 1 |
| 2026-05-19 01:30:00 | 2026-05-21 08:00:00 | 2 days 06:30:00 | 4.02 | 3.85 | 6000 | -1062.60 | -4.40 | 网格 2 |
| 2026-05-21 06:15:00 | 2026-05-21 08:00:00 | 0 days 01:45:00 | 3.85 | 3.85 | 6000 | -41.58 | -0.18 | 网格 3 |
