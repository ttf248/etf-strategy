...
- 样本内收益率：-9.42%
  - 人话：整个账户从 `200000` 变成了 `180784.00`，合计亏损 `19216.00`。
- 样本内年化收益率：-42.99%
  - 人话：这是把这段样本期的结果折算成年化后的速度，用来和别的策略比较，不是说你真的持有满一年亏这么多。
- 样本内最大回撤：12.11%
  - 人话：样本里最差的时候，账户相对阶段最高点最多回撤了约 `12.11%`。
- 期末有效持仓成本：33.06
  - 人话：把已经通过网格落袋的利润扣掉后，你手里剩余持仓等效成本大约是 `33.06`。
- 相对初始建仓成本下降：9.27%
  - 人话：最开始底仓买在 `36.44`，现在等效成本被网格摊低了约 `9.27%`。
- 网格已实现收益：7536.00
  - 人话：单看“低买高卖已经完成并落袋”的网格交易，本轮一共赚了 `7536.00`。
- 完成网格循环次数：16
...
- 样本外收益率：1.05%
  - 人话：整个账户从 `200000` 变成了 `201972.00`，合计盈利 `1972.00`。
- 样本外年化收益率：19.96%
- 样本外最大回撤：5.21%
- 样本外沿用最小交易单位：200 股
- 样本外单层网格固定数量：800 股
