...
进行中。已完成多轮前端新手化：首页、导航、创建回测、报告列表、报告详情、数据准备、策略模板、系统状态和移动端开发告警均已处理过一轮；当前继续按小步补齐首页推荐归因解释、创建回测提交引导、数据准备决策提示、报告详情图表与参数解释、报告列表排序说明和自动化验证闭环能力。
...
- 模板页已补模板对比、更强的“适合谁用”排序规则和高级管理折叠；后续继续补更细的策略差异解释和参数影响提示。
