...
进行中。已完成多轮前端新手化：首页、导航、创建回测、报告列表、报告详情、数据准备、策略模板、系统状态和移动端开发告警均已处理过一轮；当前继续按小步补齐全局壳层去运维化、报告列表卡片化、系统状态页维护判断、首页推荐引导、报告详情图表阅读和自动化验证闭环能力。
...
- 创建回测页已补上最小主路径冒烟验证，后续可继续把报告页、数据准备页等关键动作也纳入自动化覆盖，并继续细化“复跑相同配置”后的参数回填提示。
