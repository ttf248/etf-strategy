...
进行中。已完成多轮前端新手化：首页、导航、创建回测、报告列表、报告详情、数据准备、策略模板、系统状态和移动端开发告警均已处理过一轮；当前继续按小步补齐模板页高级管理降级、首页推荐引导、报告详情图表阅读和自动化验证闭环能力。
...
- 首页已补自动推荐起步动作和成功摘要，后续可继续细化“为什么推荐这个标的/模板”的归因说明。
