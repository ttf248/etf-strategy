...
进行中。已完成多轮前端新手化：首页、导航、创建回测、报告列表、报告详情、数据准备、策略模板、系统状态和移动端开发告警均已处理过一轮；当前继续按小步补齐首页推荐归因解释、首页与壳层顺序统一、创建回测提交引导、数据准备决策提示、首页/数据准备页术语收口、创建回测/报告列表任务中心口径收口、主路径英文页头收尾、策略模板边界说明、系统状态页术语收口、报告详情图表与参数解释、报告列表排序说明和自动化验证闭环能力。
