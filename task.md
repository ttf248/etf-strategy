...
进行中。已完成多轮前端新手化：首页、导航、创建回测、报告列表、报告详情、数据准备、策略模板、系统状态和移动端开发告警均已处理过一轮；当前继续按小步补齐数据准备页全量覆盖明细降级、首页推荐引导、报告详情图表阅读和自动化验证闭环能力。
...
- 模板页已补模板对比和更强的“适合谁用”排序规则，后续继续补更细的策略差异解释和参数影响提示。
