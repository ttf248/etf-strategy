...
进行中。已完成多轮前端新手化：首页、导航、创建回测、报告列表、报告详情、数据准备、策略模板、系统状态和移动端开发告警均已处理过一轮；当前继续按小步补齐首页推荐归因解释、创建回测提交引导、报告详情图表与参数解释、报告列表排序说明和自动化验证闭环能力。
...
- 数据准备页已补充示例标的推荐、准备程度分层和高级明细折叠；后续继续补更细的数据完整性、停牌和覆盖异常提示，帮助新手更快完成第一次建库。
