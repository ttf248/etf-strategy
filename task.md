...
- 模板页用户可见的“高级管理”命名继续收口成“维护区”，把 `高级模板管理 / 高级管理区 / 高级管理：...` 统一成更直白的“模板维护与自定义 / 维护区 / 维护区：...”，减少新手把这块误读成后台控制台。
- 模板页抽屉提示卡继续收口“高级编辑区”说法，把 `这里是高级编辑区` 统一成“这里是详细设置区”，减少进入抽屉时再次跳回后台心智。
- 模板页参数范围空态继续收口研究口径，把 `当前策略不需要自定义寻参空间` 统一成“当前策略不需要额外设置参数范围”，减少抽屉内仍出现研究型说法。
- 模板页参数范围输入框继续收口填写提示，把 `逗号分隔，例如 0.01,0.02,0.03` 统一成“用逗号分开填写，例如 0.01, 0.02, 0.03”，减少偏开发式占位文案。
- 模板页样本外起点输入框继续收口占位提示，把 `日线模板使用` 统一成“只在日线模板里填写”，减少抽屉内仍残留过短、像字段备注的占位文案。
- 模板页回看窗口字段继续收口研究口径，把 `样本内天数` 统一成“先回看多少天历史”，让用户先理解“系统会先看多久历史来找参数”，而不是先理解样本内术语。
- 模板页、创建回测页和报告详情页继续统一验证比例字段口径，把 `留给样本外验证的比例` 统一成“最后留多少比例做验证”，减少用户需要先理解样本外术语才能填写或复盘。
- 模板页、创建回测页和报告详情页继续统一回看窗口字段口径，把 `日线样本内天数 / 回看天数` 统一成“先回看多少天历史 / 回看多少天历史”，减少同一参数跨页切换时又回到研究术语。
- 模板页和报告详情页继续统一验证起点字段口径，把 `从哪一天开始算样本外` 统一成“从哪一天开始单独验证”，优先说明这一天之后的数据会被单独拿出来验证。
- 模板页、创建回测页和报告详情页继续统一并行寻参字段口径，把 `同时尝试的参数组数` 统一成“同时试几组参数”，减少用户需要先理解“参数组数”这种研究平台说法。
- 模板页、创建回测页和报告详情页继续统一成交假设取值口径，把 `实盘口径 / 研究口径` 统一成“更接近真实交易 / 先看理想情况”，优先帮助新手先理解两种假设的使用场景。
- 报告列表和报告详情页继续统一结果标签口径，把 `样本外收益 / 样本外盈亏` 统一成“单独验证收益 / 单独验证盈亏”，优先说明这组数字对应的是单独拿出来验证的那段结果。
- 报告列表和报告详情页继续统一结果解释口径，把 `样本外收益为正/负`、`样本外阶段`、`样本外从...到...` 这一组判断句统一翻成“单独验证...”说法，减少首屏解释和正文标签还在混用两套语言。
- 报告详情页交易活跃度空态说明继续统一口径，把 `样本外阶段没有形成完整交易` 收成“单独验证阶段没有形成完整交易”，收掉结果解释区最后一处同类残留。
- 报告详情页基础信息卡继续收口研究口径，把 `样本区间` 改成“这次用到的行情区间”，让用户直接知道这段时间就是本次回测实际读到的行情范围。
- 报告详情页高级参数折叠区继续收口后台口径，把 `高级参数明细 / 全部参数字段 / 模板快照` 统一成“补充字段 / 全部参数 / 模板来源快照”，减少结果页又回到开发排查视角。
- 报告详情页高级参数映射继续收口研究标签，把 `回测场景 / 策略评分` 统一成“这次测试的场景 / 这次结果评分”，让高级区字段更像阅读提示，而不是内部术语。
- 报告详情页事件明细标签继续收口内部字段名，把 `事件类型 / 触发价` 统一成“这次发生了什么 / 触发时价格”，让事件流水更像可读说明而不是 payload 键名。
- 报告详情页高级参数映射继续收口内部字段名，把 `锚定日期 / 基础份额` 统一成“从哪一天开始对齐 / 基础持仓份额”，让高级区字段更接近实际含义而不是历史命名。
- 报告详情页高级参数和事件明细继续统一价格高点标签，把 `入场价格 / 阶段高点时间 / 阶段高点价格 / 估算成交价` 统一成“入场时价格 / 这轮最高点时间 / 这轮最高点价格 / 大概成交价格”，减少字段名继续带出内部分析腔。
- 报告详情页高级参数映射继续收口技术标签，把 `停手冷却 K 线数` 统一成“停手后先等多少根 K 线”，让字段名更像动作说明而不是策略变量名。
- 报告详情页高级参数映射继续收口网格配置标签，把 `网格层数 / 网格模式 / 左侧行情时怎么处理` 统一成“最多开几层网格 / 网格怎么算 / 行情先走弱时怎么处理”，让字段名更像配置说明。
...
- 模板页用户可见的“高级管理”命名已继续统一成“维护区”口径；后续再总扫模板页是否还残留零散“高级管理”说法。
- 模板页抽屉提示卡已继续统一成“详细设置区”口径；后续再总扫抽屉内部是否还残留零散“高级编辑”说法。
- 模板页参数范围空态已继续统一成更自然中文；后续再总扫抽屉内部是否还残留零散研究口径。
- 模板页参数范围输入框占位已继续统一成更自然中文；后续再总扫抽屉内部是否还残留偏开发式填写提示。
- 模板页样本外起点输入框占位已继续统一成更自然中文；后续再总扫抽屉内部是否还残留零散字段备注式提示。
- 模板页回看窗口字段已继续统一成更自然中文；后续再总扫抽屉内部是否还残留零散研究术语标签。
- 验证比例字段已继续在模板页、创建回测页和报告详情页统一成更自然中文；后续再总扫跨页参数说明里是否还残留零散样本外术语。
- 回看窗口字段已继续在模板页、创建回测页和报告详情页统一成更自然中文；后续再总扫跨页参数说明里是否还残留零散样本内术语。
- 验证起点字段已继续在模板页和报告详情页统一成更自然中文；后续再总扫跨页参数说明里是否还残留零散样本外起点术语。
- 并行寻参字段已继续在模板页、创建回测页和报告详情页统一成更自然中文；后续再总扫跨页参数说明里是否还残留零散研究平台口径。
- 成交假设取值已继续在模板页、创建回测页和报告详情页统一成更自然中文；后续再总扫跨页说明里是否还残留零散研究口径命名。
- 报告列表和报告详情页的结果标签已继续统一成更自然中文；后续再总扫结果解释段落里是否还残留零散“样本外收益”术语。
- 报告列表和报告详情页的结果解释句已继续统一成“单独验证”口径；后续再总扫长段复盘文案里是否还残留零散“样本外阶段”说法。
- 报告详情页交易活跃度空态说明已继续统一成“单独验证”口径；后续再总扫结果解释区是否还残留零散样本外术语。
- 报告详情页基础信息卡里的行情区间标签已继续统一成更自然中文；后续再总扫基础信息区是否还残留零散研究口径。
- 报告详情页高级参数折叠区标题已继续统一成更自然中文；后续再总扫高级区是否还残留零散后台排查口径。
- 报告详情页高级参数里的场景与评分标签已继续统一成更自然中文；后续再总扫高级区字段名是否还残留零散研究术语。
- 报告详情页事件明细里的字段标签已继续统一成更自然中文；后续再总扫交易与事件区是否还残留零散内部键名口径。
- 报告详情页高级参数里的锚定日期与基础份额标签已继续统一成更自然中文；后续再总扫高级区字段名是否还残留零散历史命名。
- 报告详情页高级参数和事件明细里的价格高点标签已继续统一成更自然中文；后续再总扫高级区与事件区是否还残留零散分析腔字段名。
- 报告详情页高级参数里的停手冷却标签已继续统一成更自然中文；后续再总扫高级区字段名是否还残留零散策略变量名。
- 报告详情页高级参数里的网格与左侧行情标签已继续统一成更自然中文；后续再总扫高级区字段名是否还残留零散策略术语。
...
继续按最小边界总扫报告详情页高级参数区，只处理 `benchmark / Benchmark` 同一概念的一组标签和值映射，不扩大到其他字段。

### 修改内容

- `frontend/src/components/report-detail-view.tsx`
  - 将 `benchmark`、`Benchmark` 的展示标签从“对照基准”改为“和谁做对照”。
  - 将 `buy_hold`、`cash_idle` 的展示值改为“买入后一直拿着”“空仓不买”。
- `doc/frontend-ux-audit.md`
  - 记录本轮文案收口背景、边界和取舍。

### 设计取舍

- 保留后端字段名和取值 `benchmark / Benchmark / buy_hold / cash_idle` 不变，只调整前端展示文案，避免影响报告快照和接口兼容性。
- 标签采用“和谁做对照”，优先帮助新手理解这个字段是在回答“策略是跟什么方案比较”，而不是继续暴露研究口径“基准”。
...
继续按创建回测页局部体验边界优化，只处理“最近回测任务”卡片里最主要的结果按钮文案，把它从“查看某个页面”收成更像“下一步先去看结果”。

### 修改内容

- `frontend/src/components/backtests-view.tsx`
  - 将最近任务卡主按钮从“查看报告 / 查看报告列表 / 等待报告”改成“先看这份结果 / 去结果列表里找 / 先等结果出来”。
- `doc/frontend-ux-audit.md`
  - 记录本轮体验收口背景、边界和取舍。

### 设计取舍

- 不删除跳转报告的主按钮，也不改变卡片状态判断、报告链接选择或次级“取消 / 重试”动作，只调整最主要的结果按钮文案。
- 这轮不改摘要横幅、错误提示、完整历史入口或批量处理，保持提交边界清晰。
...
继续按创建回测页局部体验边界优化，只处理“最近回测任务”摘要横幅里的主动作文案，把“打开最近成功报告”这种页面跳转口吻收成更像“先去看最新跑成的结果”。

### 修改内容

- `frontend/src/components/backtests-view.tsx`
  - 将摘要横幅主按钮从“打开最近成功报告”改成“先看最新跑成这份”。
- `doc/frontend-ux-audit.md`
  - 记录本轮体验收口背景、边界和取舍。

### 设计取舍

- 不删除直达最近成功报告的主按钮，也不改次按钮“去看报告列表”、任务摘要统计或最近任务卡，只调整这一处主动作文案。
...
继续按创建回测页局部体验边界优化，只处理“完整历史”工具条里的批量动作文案，把“取消所选 / 重试所选”这类后台任务中心按钮收成更贴近“停掉所选这些 / 按原再跑所选”的用户操作表达。

### 修改内容

- `frontend/src/components/backtests-view.tsx`
  - 将完整历史工具条批量动作从“取消所选 / 重试所选”改成“停掉所选这些 / 按原再跑所选”。
- `doc/frontend-ux-audit.md`
  - 记录本轮体验收口背景、边界和取舍。

### 设计取舍

- 不改按钮启用条件、请求逻辑、行内动作或其他列，只调整完整历史工具条批量动作这一组文案。
- 这轮不改摘要横幅、最近任务卡或表格底部说明，保持提交边界清晰。
...
继续按创建回测页局部体验边界优化，只处理最近任务卡 `succeeded` 但暂时没有报告链接时的阅读提示，把“任务执行完成，但报告还没挂上”这类后台链路口吻收成更贴近用户等待结果出现时的自然表达。

### 修改内容

- `frontend/src/components/backtests-view.tsx`
  - 将成功但未挂结果提示从“任务执行完成，但报告还没挂上时，先去报告列表刷新确认。”改成“这轮已经跑完；如果结果还没马上出现，就去结果列表刷新确认。”。
- `doc/frontend-ux-audit.md`
  - 记录本轮体验收口背景、边界和取舍。

### 设计取舍

- 不改变任务状态判断逻辑，也不改 `queued / running / failed / cancelled` 其他分支，只调整成功但未挂结果这一条提示文案。
...
继续按创建回测页局部体验边界优化，只处理“完整历史”桌面表格里的提交时间列表头，把“提交时间”这类后台字段名收成更像用户在核对记录时会直接问的问题。

### 修改内容

- `frontend/src/components/backtests-view.tsx`
  - 将完整历史表格提交时间列从“提交时间”改成“什么时候提交”。
- `doc/frontend-ux-audit.md`
  - 记录本轮体验收口背景、边界和取舍。

### 设计取舍

- 不改时间值本身，也不动完成时间列、动作按钮或其他列，只调整提交时间列表头这一处文案。
- 这轮不改摘要横幅、最近任务卡或状态提示，保持提交边界清晰。
...
继续按创建回测页局部体验边界优化，只处理“完整历史”桌面表格里的完成时间列表头，把“完成时间”这类后台字段名收成更像用户在核对记录时会直接问的问题。

### 修改内容

- `frontend/src/components/backtests-view.tsx`
  - 将完整历史表格完成时间列从“完成时间”改成“什么时候结束”。
- `doc/frontend-ux-audit.md`
  - 记录本轮体验收口背景、边界和取舍。

### 设计取舍

- 不改时间值本身，也不动提交时间列、动作按钮或其他列，只调整完成时间列表头这一处文案。
- 这轮不改摘要横幅、最近任务卡或状态提示，保持提交边界清晰。
...
继续按创建回测页局部体验边界优化，只处理最近任务卡主按钮在“没有报告链接”时的兜底口径，避免把失败或已停掉的任务也一律写成“先等结果出来”。

### 修改内容

- `frontend/src/components/backtests-view.tsx`
  - 新增最近任务卡主按钮文案映射。
  - 有报告时仍显示“先看这份结果”；已跑完但暂时没挂报告时显示“去结果列表里找”。
  - 失败任务改成禁用显示“这次没生成结果”；已停掉任务改成禁用显示“这轮已经停掉”；只有还没开始或还在跑时才继续显示“先等结果出来”。

### 设计取舍

- 不改任务状态判断逻辑、主按钮跳转目标、次级动作按钮、错误提示或完整历史表格，只调整最近任务卡主按钮在无结果时的显示文案。
- 这轮不把桌面完整历史或其他页面一起改成同样逻辑，避免扩大提交边界。
...
继续按报告页局部体验边界优化，只处理首屏统计区里“当前最高收益”这张卡，把偏排行榜/结果中心的说法收成更像普通用户在浏览这批结果时会直接理解的问题。

### 修改内容

- `frontend/src/components/reports-view.tsx`
  - 将统计卡标签从“当前最高收益”改成“这批里最赚钱的”。

### 设计取舍

- 不改收益计算逻辑、卡片数值、排序规则、其他统计卡或下方报告列表，只调整这一张统计卡标签。
- 这轮不顺手处理“最近新结果”或其他统计卡口径，避免扩大提交边界。
...
继续按报告页局部体验边界优化，只处理首屏统计区里“当前能看的报告”这张卡，把偏结果中心统计面板的说法收成更像普通用户在浏览当前这批结果时会直接理解的表达。

### 修改内容

- `frontend/src/components/reports-view.tsx`
  - 将统计卡标签从“当前能看的报告”改成“现在这批里有”。

### 设计取舍

- 不改报告数量计算、卡片数值、其他统计卡、排序规则或下方报告列表，只调整这一张统计卡标签。
- 这轮不顺手处理“最近新结果”或“你标记过的”等其他统计卡口径，避免扩大提交边界。
