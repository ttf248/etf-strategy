...
- `market_data_runtime.tdx`：`STRATEGY_STUDIO_TDX_VIPDOC` 或 `STRATEGY_STUDIO_TDX_CONFIG_PATH` 解析出的 `vipdoc` 路径、来源、是否存在、第一层市场目录摘要、按 `1d / 1m / 5m` 和市场目录汇总的可见 K 线文件数，以及 `manifest_coverage` 导入覆盖摘要。`manifest_coverage` 会按数据库 `source_file_manifests` 与实时 `vipdoc` 库存比对，返回整体与分周期的 `inventory_files / imported_files / pending_files / coverage_pct`，并在可识别市场目录时补充 `by_market` 维度。
