...
当前前复权算法沿用外部 C++ 版本的核心思路：先把公司行动合并成日期级仿射事件，再反向预计算后缀 `A/B`，最后让“已按交易日升序的原始日线”和“已按起止日升序的公式区间”走线性双指针匹配，把整段日线一次性映射到对应区间，避免回到 Python 旧链路那种每条 K 线重复扫描全部事件或频繁二分查区间的做法。
