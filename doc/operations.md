...
该命令会直接输出数据库迁移状态、后台心跳、任务队列，以及 Yahoo / 通达信 / Tushare 的运行时配置摘要。若数据库异常、`worker / scheduler` 没有可见心跳，或某条数据链路处于 `misconfigured`，命令会返回非零退出码，适合在日常排障或自动化脚本里先做前置检查。当前通达信摘要除了 `vipdoc` 文件库存，还会额外打印 `TDX 导入覆盖` 和 `TDX 周期覆盖`，用于对比数据库 `source_file_manifests` 与实时 `vipdoc` 库存，快速判断当前还差多少文件未导入、哪一个周期推进最慢。若当前机器是 Windows，且系统代理里已经存在可复制的代理候选，但项目尚未通过 `--proxy` 或 `STRATEGY_STUDIO_PROXY` 显式启用，输出里会额外给出 `Yahoo 代理候选` 和提示语，便于直接把本机代理接到 Yahoo 下载链路上。
...
维护页 `/platform` 现在还会直接显示多数据源运行配置摘要：数据库连接状态、通达信 `vipdoc` 解析结果、按市场/周期汇总的可见 K 线文件数、数据库 manifest 相对库存的导入覆盖率、Tushare token 是否存在，以及 Yahoo 是否显式配置代理。若本机存在系统代理候选但项目没显式启用，`check-runtime` 也会直接提示“当前不会自动使用”，避免误判成 Yahoo 本身不可达。若怀疑平台“命令能跑但前端任务总失败”，建议先看这里，再决定是否继续翻日志或直接重试导入任务。
