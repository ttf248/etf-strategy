...
现在这两条配置会弹出系统外部终端窗口执行，不走 VS Code 内置终端。

为了尽量避免 Windows 外部终端里的中文乱码，项目入口 `main.py` 现在会主动把控制台切到 UTF-8，VS Code 调试环境也显式设置了：
...
所以日常看执行进度，主要看弹出的控制台窗口；只有排查问题时，再去看 `log/` 目录下的日志文件。
