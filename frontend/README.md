...
- `/market-data`：数据准备，先设定当前标的，再在同一页查看 Yahoo、通达信原始 `all / 1d / 1m / 5m`、Tushare 公司行动和通达信前复权的任务概览；其中 Yahoo 批量入口会默认走内置 `yahoo_global_active_100` 样本池，TDX 卡片可直接切换 `all / 1d / 1m / 5m` 发起导入，同时保留可直接回测样本的覆盖检查、推荐周期和高级明细。
