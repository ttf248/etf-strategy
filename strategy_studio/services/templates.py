...
from strategy_studio.config import DEFAULT_MINUTE_INTERVAL
from strategy_studio.db.models import StrategyParameterTemplate
from strategy_studio.db.session import open_session
from strategy_studio.repositories.templates import (
...
def build_seed_templates() -> list[TemplateSeed]:
    ...
    return [
        ...
        TemplateSeed(
            template_key="ma_cross_1d_realistic_default",
            template_name="双均线趋势-日线默认模板",
            ...
            jobs=1,
            execution_overrides_json=base_execution,
            parameter_space_json=default_parameter_space_for_template("ma_cross", "1d"),
            description="用短长均线金叉/死叉验证趋势跟随的日线默认模板。",
        ),
        TemplateSeed(
            template_key="macd_trend_1d_realistic_default",
            template_name="MACD 趋势-日线默认模板",
            strategy_kind="macd_trend",
            interval="1d",
            execution_profile="realistic",
            validation_start=DEFAULT_VALIDATION_START,
            lookback_days=DEFAULT_LOOKBACK_DAYS,
            ...
            jobs=1,
            execution_overrides_json=base_execution,
            parameter_space_json=default_parameter_space_for_template("macd_trend", "1d"),
            description="用 MACD 金叉与柱体确认验证中期动量延续性的日线默认模板。",
        ),
        TemplateSeed(
            template_key="donchian_breakout_1d_realistic_default",
            template_name="唐奇安突破-日线默认模板",
            strategy_kind="donchian_breakout",
            interval="1d",
            execution_profile="realistic",
            validation_start=DEFAULT_VALIDATION_START,
            lookback_days=DEFAULT_LOOKBACK_DAYS,
            ...
            jobs=1,
            execution_overrides_json=base_execution,
            parameter_space_json=default_parameter_space_for_template("donchian_breakout", "1d"),
            description="用历史高点突破与退出通道验证趋势延续性的日线默认模板。",
        ),
        TemplateSeed(
            template_key="volume_breakout_1d_realistic_default",
            template_name="放量突破-日线默认模板",
            strategy_kind="volume_breakout",
            interval="1d",
            execution_profile="realistic",
            validation_start=DEFAULT_VALIDATION_START,
            lookback_days=DEFAULT_LOOKBACK_DAYS,
            ...
            jobs=1,
            execution_overrides_json=base_execution,
            parameter_space_json=default_parameter_space_for_template("volume_breakout", "1d"),
            description="用价格突破叠加量能放大确认趋势延续性的日线默认模板。",
        ),
        TemplateSeed(
            template_key="bollinger_reversion_1d_realistic_default",
            template_name="布林带均值回归-日线默认模板",
            ...
            jobs=1,
            execution_overrides_json=base_execution,
            parameter_space_json=default_parameter_space_for_template("bollinger_reversion", "1d"),
            description="用布林带下轨与均值回归节奏验证震荡环境的日线默认模板。",
        ),
        TemplateSeed(
