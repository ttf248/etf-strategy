...
1. 收盘价向上突破过去 N 根 K 线高点时买入。
2. 跌回较短退出通道下沿，或触发固定止损时卖出。
3. 统一复用现有执行口径、仓位限制、费用和买入持有对照。

它比双均线更强调“价格突破”本身，适合作为经典趋势跟随的另一条基线。
"""
...
_DONCHIAN_OPTIMIZATION_CONTEXT: dict[str, object] | None = None
...
def _prepare_features(data: pd.DataFrame, breakout_window: int, exit_window: int) -> pd.DataFrame:
    frame = data.copy()
...
def run_donchian_breakout_backtest(
    ...
    """运行一次唐奇安突破回测。"""
    if data.empty:
        raise ValueError("唐奇安突破回测需要非空行情数据。")
    ...
    frame = _prepare_features(data, breakout_window=breakout_window, exit_window=exit_window)
    cash = total_capital
    position_units = 0
    ...
    for timestamp, row in frame.iterrows():
        ...
        if position_units > 0:
            ...
            if stop_loss_pct > 0 and open_return_pct <= -stop_loss_pct:
                ...
                exit_reason = "donchian_exit_sell"
                channel_exit_events += 1
        ...
        if entry_allowed:
            if cooldown_remaining > 0:
                ...
                if units <= 0:
                    raise ValueError(
                        f"唐奇安突破预算不足以买入 1 手: symbol={symbol} lot_size={lot_size} price={close_price:.2f}"
                    )
    ...
    summary = {
        ...
        "DonchianEntryEvents": entry_count,
        "DonchianExitEvents": channel_exit_events,
...
def _set_donchian_optimization_context(context: dict[str, object] | None) -> None:
    global _DONCHIAN_OPTIMIZATION_CONTEXT
    _DONCHIAN_OPTIMIZATION_CONTEXT = context


def _run_donchian_candidate_task(params: dict[str, object]) -> dict[str, object]:
    context = _DONCHIAN_OPTIMIZATION_CONTEXT
    if context is None:
        raise RuntimeError("唐奇安突破寻参上下文尚未初始化。")
    run_result = run_donchian_breakout_backtest(
        data=context["data"],
...
def optimize_donchian_breakout_parameters(
    data: pd.DataFrame,
    ...
    candidate_params = [
        ...
        for breakout_window, exit_window, confirm_buffer_pct, stop_loss_pct in product(
            breakout_windows,
            exit_windows,
            confirm_buffers,
            stop_losses,
    ...
    if not candidate_params:
        raise ValueError("唐奇安突破参数空间为空，或退出窗口全部大于等于突破窗口。")
    ...
    if effective_jobs == 1 or len(candidate_params) <= 1:
        _set_donchian_optimization_context(context)
        try:
            candidate_runs = [_run_donchian_candidate_task(params) for params in candidate_params]
        finally:
            _set_donchian_optimization_context(None)
    else:
        with ProcessPoolExecutor(
            max_workers=effective_jobs,
            initializer=_set_donchian_optimization_context,
            initargs=(context,),
        ) as executor:
            candidate_runs = list(executor.map(_run_donchian_candidate_task, candidate_params))
    ...
    if results.empty:
        raise ValueError("唐奇安突破参数搜索未产生任何结果。")
    ...
    if best_run is None:
        raise ValueError("无法回取最优唐奇安突破参数对应的回测结果。")
