...
这里补上一类最常见、也最容易向非量化用户解释的日线动量策略：

1. MACD 线向上穿越信号线，且柱体强度达到确认阈值时买入。
2. MACD 线向下穿越信号线，或触发固定止损时卖出。
3. 统一复用项目已有的手续费、滑点、仓位上限、冷却期和买入持有对照。

它和双均线、唐奇安突破同属趋势跟随，但更强调“动量转强”的节奏判断。
"""
...
_MACD_OPTIMIZATION_CONTEXT: dict[str, object] | None = None
...
def _prepare_features(data: pd.DataFrame, fast_window: int, slow_window: int, signal_window: int) -> pd.DataFrame:
    frame = data.copy()
    if not isinstance(frame.index, pd.DatetimeIndex):
        frame.index = pd.to_datetime(frame.index)
    frame = frame.sort_index()
    frame["FastEma"] = frame["Close"].ewm(span=fast_window, adjust=False, min_periods=fast_window).mean()
    frame["SlowEma"] = frame["Close"].ewm(span=slow_window, adjust=False, min_periods=slow_window).mean()
    frame["MacdLine"] = frame["FastEma"] - frame["SlowEma"]
    frame["SignalLine"] = frame["MacdLine"].ewm(span=signal_window, adjust=False, min_periods=signal_window).mean()
    frame["Histogram"] = frame["MacdLine"] - frame["SignalLine"]
    frame["HistogramPct"] = np.where(
        frame["Close"] != 0,
        frame["Histogram"] / frame["Close"] * 100,
        0.0,
    )
    return frame
...
def run_macd_trend_backtest(
    ...
    """运行一次 MACD 趋势回测。"""
    if data.empty:
        raise ValueError("MACD 趋势回测需要非空行情数据。")
    ...
    histogram_confirm_pct = float(params["histogram_confirm_pct"])
    stop_loss_pct = float(params["stop_loss_pct"])
    if fast_window <= 0 or slow_window <= 0 or signal_window <= 0:
        raise ValueError("MACD 各窗口必须为正整数。")
    if fast_window >= slow_window:
        raise ValueError("MACD 快线窗口必须小于慢线窗口。")
    if histogram_confirm_pct < 0:
        raise ValueError("MACD 柱体确认阈值不能为负数。")
    if stop_loss_pct < 0:
        raise ValueError("止损比例不能为负数。")

    frame = _prepare_features(
        data,
    ...
    for index, (timestamp, row) in enumerate(frame.iterrows()):
        ...
        macd_line = row["MacdLine"]
        signal_line = row["SignalLine"]
        histogram_pct = float(row["HistogramPct"]) if pd.notna(row["HistogramPct"]) else 0.0
        can_read_signal = previous_row is not None and pd.notna(macd_line) and pd.notna(signal_line)

        crossed_up = False
        crossed_down = False
        if can_read_signal and previous_row is not None:
            prev_macd = previous_row["MacdLine"]
            prev_signal = previous_row["SignalLine"]
            if pd.notna(prev_macd) and pd.notna(prev_signal):
                crossed_up = bool(float(prev_macd) <= float(prev_signal) and float(macd_line) > float(signal_line))
                crossed_down = bool(float(prev_macd) >= float(prev_signal) and float(macd_line) < float(signal_line))
            else:
                # 信号线刚刚开始有效时，没有上一根完整信号可比。
                # 这时若当前已经处于多头/空头结构，允许把它当作首个有效趋势状态。
                crossed_up = bool(float(macd_line) > float(signal_line))
                crossed_down = bool(float(macd_line) < float(signal_line))
        ...
        entry_allowed = position_units == 0 and crossed_up and histogram_pct >= histogram_confirm_pct
        if entry_allowed:
            if cooldown_remaining > 0:
                ...
                if units <= 0:
                    raise ValueError(f"MACD 趋势预算不足以买入 1 手: symbol={symbol} lot_size={lot_size} price={close_price:.2f}")
        ...
        history_rows.append(
            {
                "Date": format_timestamp(timestamp),
                "Close": close_price,
                "FastEma": float(row["FastEma"]) if pd.notna(row["FastEma"]) else 0.0,
                "SlowEma": float(row["SlowEma"]) if pd.notna(row["SlowEma"]) else 0.0,
                "MacdLine": float(macd_line) if pd.notna(macd_line) else 0.0,
                "SignalLine": float(signal_line) if pd.notna(signal_line) else 0.0,
                "HistogramPct": histogram_pct,
    ...
    summary = {
        ...
        "slow_window": slow_window,
        "signal_window": signal_window,
        "histogram_confirm_pct": histogram_confirm_pct,
        "stop_loss_pct": stop_loss_pct,
        "FastWindow": fast_window,
        "SlowWindow": slow_window,
        "SignalWindow": signal_window,
        "HistogramConfirmPct": histogram_confirm_pct,
        "MacdEntryEvents": entry_count,
        "MacdExitEvents": macd_exit_events,
...
def _set_macd_optimization_context(context: dict[str, object] | None) -> None:
    global _MACD_OPTIMIZATION_CONTEXT
    _MACD_OPTIMIZATION_CONTEXT = context


def _run_macd_candidate_task(params: dict[str, object]) -> dict[str, object]:
    context = _MACD_OPTIMIZATION_CONTEXT
    if context is None:
        raise RuntimeError("MACD 趋势寻参上下文尚未初始化。")
    run_result = run_macd_trend_backtest(
        data=context["data"],
...
def optimize_macd_trend_parameters(
    data: pd.DataFrame,
    ...
    """穷举 MACD 趋势参数，并按稳健性排序。"""
    execution = execution_config or build_execution_config("research")
    fast_windows = [int(item) for item in parameter_space["fast_window"]]
    slow_windows = [int(item) for item in parameter_space["slow_window"]]
    signal_windows = [int(item) for item in parameter_space["signal_window"]]
    histogram_confirms = [float(item) for item in parameter_space["histogram_confirm_pct"]]
    ...
    candidate_params = [
        ...
        for fast_window, slow_window, signal_window, histogram_confirm_pct, stop_loss_pct in product(
            fast_windows,
            slow_windows,
            signal_windows,
            histogram_confirms,
    ...
    if not candidate_params:
        raise ValueError("MACD 趋势参数空间为空，或快线窗口全部大于等于慢线窗口。")
    ...
    if effective_jobs == 1 or len(candidate_params) <= 1:
        _set_macd_optimization_context(context)
        try:
            candidate_runs = [_run_macd_candidate_task(params) for params in candidate_params]
        finally:
            _set_macd_optimization_context(None)
    else:
        with ProcessPoolExecutor(
            max_workers=effective_jobs,
            initializer=_set_macd_optimization_context,
            initargs=(context,),
        ) as executor:
            candidate_runs = list(executor.map(_run_macd_candidate_task, candidate_params))
        ...
        json.dumps(
            {
                ...
                "histogram_confirm_pct": row["histogram_confirm_pct"],
    ...
    if results.empty:
        raise ValueError("MACD 趋势参数搜索未产生任何结果。")
    best_row = results.iloc[0]
    best_key = json.dumps(
        {
            "fast_window": best_row["fast_window"],
            "slow_window": best_row["slow_window"],
            "signal_window": best_row["signal_window"],
            "histogram_confirm_pct": best_row["histogram_confirm_pct"],
    ...
    if best_run is None:
        raise ValueError("无法回取最优 MACD 趋势参数对应的回测结果。")
