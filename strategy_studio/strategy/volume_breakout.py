...
这里补上一类更贴近主流技术分析话语体系的日线趋势策略：

1. 收盘价向上突破过去 N 根 K 线高点时，只在成交量同步放大时买入。
2. 跌回退出通道或触发固定止损时卖出。
3. 统一复用项目已有的费用、滑点、仓位上限、冷却期和买入持有对照。

它和唐奇安突破同属趋势跟随，但额外要求“量能确认”，适合作为更偏交易员视角的突破基线。
"""
...
_VOLUME_BREAKOUT_OPTIMIZATION_CONTEXT: dict[str, object] | None = None
...
def _prepare_features(data: pd.DataFrame, breakout_window: int, exit_window: int, volume_window: int) -> pd.DataFrame:
    frame = data.copy()
    if not isinstance(frame.index, pd.DatetimeIndex):
        frame.index = pd.to_datetime(frame.index)
    frame = frame.sort_index()
    frame["BreakoutHigh"] = frame["Close"].rolling(window=breakout_window, min_periods=breakout_window).max().shift(1)
    frame["ExitLow"] = frame["Close"].rolling(window=exit_window, min_periods=exit_window).min().shift(1)
    frame["VolumeAverage"] = frame["Volume"].rolling(window=volume_window, min_periods=volume_window).mean().shift(1)
    frame["VolumeRatio"] = np.where(frame["VolumeAverage"] > 0, frame["Volume"] / frame["VolumeAverage"], 0.0)
    return frame
...
def run_volume_breakout_backtest(
    ...
    """运行一次放量突破回测。"""
    if data.empty:
        raise ValueError("放量突破回测需要非空行情数据。")
    ...
    confirm_buffer_pct = float(params["confirm_buffer_pct"])
    stop_loss_pct = float(params["stop_loss_pct"])
    if breakout_window <= 1 or exit_window <= 0:
        raise ValueError("突破窗口必须大于 1，退出窗口必须为正整数。")
    if exit_window >= breakout_window:
        raise ValueError("退出窗口必须小于突破窗口。")
    if volume_window <= 0:
        raise ValueError("成交量均值窗口必须为正整数。")
    if volume_multiplier < 0:
        raise ValueError("成交量放大倍数不能为负数。")
    if confirm_buffer_pct < 0:
        raise ValueError("突破确认比例不能为负数。")
    if stop_loss_pct < 0:
    ...
    for timestamp, row in frame.iterrows():
        ...
        breakout_high = row["BreakoutHigh"]
        exit_low = row["ExitLow"]
        volume_average = row["VolumeAverage"]
        volume_ratio = float(row["VolumeRatio"]) if pd.notna(row["VolumeRatio"]) else 0.0
        ...
        if position_units > 0:
            ...
            if stop_loss_pct > 0 and open_return_pct <= -stop_loss_pct:
                ...
                stop_loss_events += 1
            elif pd.notna(exit_low) and close_price <= float(exit_low):
                exit_reason = "volume_breakout_exit_sell"
                breakout_exit_events += 1
        ...
        entry_allowed = (
            position_units == 0
            and pd.notna(breakout_high)
            and pd.notna(volume_average)
            and close_price >= float(breakout_high) * (1 + confirm_buffer_pct)
        ...
        if entry_allowed:
            if cooldown_remaining > 0:
                ...
                if units <= 0:
                    raise ValueError(
                        f"放量突破预算不足以买入 1 手: symbol={symbol} lot_size={lot_size} price={close_price:.2f}"
                    )
        ...
        history_rows.append(
            {
                "Date": format_timestamp(timestamp),
                "Close": close_price,
                "BreakoutHigh": float(breakout_high) if pd.notna(breakout_high) else 0.0,
                "ExitLow": float(exit_low) if pd.notna(exit_low) else 0.0,
                "VolumeAverage": float(volume_average) if pd.notna(volume_average) else 0.0,
    ...
    summary = {
        ...
        "ConfirmBufferPct": confirm_buffer_pct * 100,
        "VolumeBreakoutEntryEvents": entry_count,
        "VolumeBreakoutExitEvents": breakout_exit_events,
...
def _set_volume_breakout_optimization_context(context: dict[str, object] | None) -> None:
    global _VOLUME_BREAKOUT_OPTIMIZATION_CONTEXT
    _VOLUME_BREAKOUT_OPTIMIZATION_CONTEXT = context


def _run_volume_breakout_candidate_task(params: dict[str, object]) -> dict[str, object]:
    context = _VOLUME_BREAKOUT_OPTIMIZATION_CONTEXT
    if context is None:
        raise RuntimeError("放量突破寻参上下文尚未初始化。")
    run_result = run_volume_breakout_backtest(
        data=context["data"],
...
def optimize_volume_breakout_parameters(
    data: pd.DataFrame,
    ...
    """穷举放量突破参数，并按稳健性排序。"""
    execution = execution_config or build_execution_config("research")
    breakout_windows = [int(item) for item in parameter_space["breakout_window"]]
    exit_windows = [int(item) for item in parameter_space["exit_window"]]
    volume_windows = [int(item) for item in parameter_space["volume_window"]]
    volume_multipliers = [float(item) for item in parameter_space["volume_multiplier"]]
    confirm_buffers = [float(item) for item in parameter_space["confirm_buffer_pct"]]
    ...
    candidate_params = [
        ...
        for breakout_window, exit_window, volume_window, volume_multiplier, confirm_buffer_pct, stop_loss_pct in product(
            breakout_windows,
            exit_windows,
            volume_windows,
            volume_multipliers,
        ...
        if int(exit_window) < int(breakout_window)
    ]
    if not candidate_params:
        raise ValueError("放量突破参数空间为空，或退出窗口全部大于等于突破窗口。")
    ...
    if effective_jobs == 1 or len(candidate_params) <= 1:
        _set_volume_breakout_optimization_context(context)
        try:
            candidate_runs = [_run_volume_breakout_candidate_task(params) for params in candidate_params]
        finally:
            _set_volume_breakout_optimization_context(None)
    else:
        with ProcessPoolExecutor(
            max_workers=effective_jobs,
            initializer=_set_volume_breakout_optimization_context,
            initargs=(context,),
        ) as executor:
            candidate_runs = list(executor.map(_run_volume_breakout_candidate_task, candidate_params))

    rows = [run_result["summary"] for run_result in candidate_runs]
    run_records = {
        json.dumps(
            {
                "breakout_window": row["breakout_window"],
                "exit_window": row["exit_window"],
                "volume_window": row["volume_window"],
                "volume_multiplier": row["volume_multiplier"],
                "confirm_buffer_pct": row["confirm_buffer_pct"],
    ...
    if results.empty:
        raise ValueError("放量突破参数搜索未产生任何结果。")
    best_row = results.iloc[0]
    best_key = json.dumps(
        {
            "breakout_window": best_row["breakout_window"],
            "exit_window": best_row["exit_window"],
            "volume_window": best_row["volume_window"],
            "volume_multiplier": best_row["volume_multiplier"],
            "confirm_buffer_pct": best_row["confirm_buffer_pct"],
    ...
    if best_run is None:
        raise ValueError("无法回取最优放量突破参数对应的回测结果。")
